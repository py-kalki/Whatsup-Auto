  ...
  async processIncomingMessage({ chatId, text, senderName, messageId }) {
    ...
    for (const rule of rules) {
      if (this.matchRule(rule, rawText)) {
        ...
        reply = reply.replace(/{{business_name}}/g, profile.businessName || 'WhatsAuto');
        reply = reply.replace(/{{support_email}}/g, profile.supportEmail || '');
        reply = reply.replace(/{{phone}}/g, profile.contactPhone || '');
        reply = reply.replace(/{{website}}/g, profile.websiteUrl || '');
        reply = reply.replace(/{{hours}}/g, profile.operatingHours || '');
        reply = reply.replace(/{{signature}}/g, profile.customSignature || '');
    ...
    return {
      ...
      replyText: `Hello! I received your message. Type */help* to view our options or */lead* to connect with us!`,
      reactionEmoji: null,
